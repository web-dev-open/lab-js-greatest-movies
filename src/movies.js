...
function howManyMovies(moviesArray) {
  const totalMovies = moviesArray.filter(movie => {
...
function scoresAverage(moviesArray) {
  let newArray = [];
  const getDecimals = moviesArray.filter(movies => {
    if (movies.score % 1 !== 0) {
      return newArray.push(movies.score);
    }
  });

  const totalScores = newArray.reduce((accumulator, currentValue) => {
...
console.log(scoresAverage(SampleMovies));
