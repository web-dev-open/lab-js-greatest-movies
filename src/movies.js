...
function getAllDirectors(movies) {
  ...
  return [...new Set(arrayOfDirectors)];
}
...
function scoresAverage(movies) {
  ...
  const totalScore = movies.reduce((sum,movie) => {
    return typeof movie.score === 'number' ? sum + movie.score : sum; // Ignores movies without a valid score
  }, 0);
...
function orderAlphabetically(movies) {
  const sortedMovies = [...movies]; // Create shallow copy of original array
  sortedMovies.sort((a, b) => a.title.localeCompare(b.title)); // Sort by title
